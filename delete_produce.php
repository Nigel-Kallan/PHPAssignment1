<?php
    ...
    $queryProduce = '
        SELECT produceCode, itemName, variety, origin, quantity, measure, price, imageName FROM produce WHERE produceCode = :produce_code';

    $statement = $db->prepare($queryProduce);
    $statement->bindValue(':produce_code', $produce_code);
    $statement->execute();
    $produce = $statement->fetch();
    $statement->closeCursor();

    $old_image_name = $produce['imageName'];
    $base_dir = 'images/';
